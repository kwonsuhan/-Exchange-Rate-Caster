...
raw_data = pd.read_csv(os.path.join("data", "numeric_total_1.csv"), nrows=100 if debug else None)
...
text_label = pd.read_csv(os.path.join("data", "pred_74_7.csv"))
text_label = [f(str(i)) for i in text_label['categorical_pred']]
text_label = np.array(text_label[-1])
